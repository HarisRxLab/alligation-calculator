...
function Index() {
  ...
  const result = useMemo(() => {
    ...
    return { volHigh, volLow: vol - volHigh, partsHigh, partsLow, hi, lo };
      ...
      <div className="w-full max-w-xl">
        ...
        <div className="bg-white border border-[oklch(0.88_0.03_60)] rounded-b-sm shadow-sm px-8 pt-6 pb-8">
          ...
          <div className="space-y-3 mb-6">
            <Field label="Solution A — Higher %" value={cHigh} onChange={setCHigh} placeholder="e.g. 70" suffix="%" />
            <Field label="Solution B — Lower %" value={cLow} onChange={setCLow} placeholder="e.g. 20" suffix="%" />
            <Field label="Desired concentration" value={cDesired} onChange={setCDesired} placeholder="e.g. 40" suffix="%" />
            <Field label="Total volume" value={volume} onChange={setVolume} placeholder="e.g. 500" suffix="mL" />
          </div>

          {/* Diagram */}
          <p className="text-xs uppercase tracking-widest text-[oklch(0.6_0.02_60)] mb-2">Alligation diagram</p>
          <AlligationDiagram d={diagram} />

          {/* Result */}
          <div className="mt-6 border-t border-[oklch(0.88_0.03_60)] pt-4">
            <p className="text-xs uppercase tracking-widest text-[oklch(0.6_0.02_60)] mb-3">Measure</p>
            {!result && (
              <p className="text-sm italic text-[oklch(0.65_0.02_60)]">Fill all four fields above to see the mixture.</p>
            )}
            ...
            {result && !("error" in result) && (
              <div className="space-y-2">
                <MeasureRow label="Solution A" sub={`${fmt(result.hi, 2)}%`} value={fmt(result.volHigh, 2)} unit="mL" />
                <MeasureRow label="Solution B" sub={`${fmt(result.lo, 2)}%`} value={fmt(result.volLow, 2)} unit="mL" />
                <div className="flex items-baseline gap-2 pt-1 text-sm text-[oklch(0.45_0.02_60)]">
                  <span className="font-semibold">Ratio A : B</span>
                  <span>{fmt(result.partsHigh, 2)} : {fmt(result.partsLow, 2)}</span>
                </div>
              </div>
            )}
          </div>

          {/* Clear Worksheet Button */}
          <div className="mt-6 pt-4">
            <button
              onClick={clearAll}
              className="w-full border border-[oklch(0.75_0.03_60)] text-xs tracking-widest uppercase text-[oklch(0.45_0.02_60)] py-3 px-4 hover:bg-[oklch(0.95_0.01_60)] hover:border-[oklch(0.55_0.03_60)] transition-all duration-200 font-sans font-semibold"
            >
              Clear Worksheet
            </button>
          </div>
          ...
          <div className="mt-4 flex items-center justify-between">
            <p className="text-[10px] text-[oklch(0.72_0.015_60)] leading-relaxed flex-1 pr-4">
              For educational and calculation support only. Does not replace institutional policies or independent clinical judgment.
            </p>
            <p className="text-xs text-[oklch(0.7_0.02_60)] italic whitespace-nowrap">Haris Mohamed K M</p>
          </div>
        </div>
      </div>
    </div>
...
function Field({
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <label className="text-xs uppercase tracking-widest text-[oklch(0.55_0.02_60)]">{label}</label>
      <div className="flex items-center border-b border-[oklch(0.82_0.025_60)] pb-1 gap-2">
        ...
        />
        {suffix && <span className="text-sm text-[oklch(0.6_0.02_60)]">{suffix}</span>}
      </div>
    </div>
  );
}
...
function AlligationDiagram({ d }: {
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-sm mx-auto" aria-label="Alligation diagram">
        ...
        {Object.entries(cells).map(([key, c]) => (
          <g key={key} transform={`translate(${c.x},${c.y})`}>
            ...
            <text textAnchor="middle" dy="-4" fontSize="11" fontFamily="serif" fill={ink} fontWeight="600">{c.value}</text>
            <text textAnchor="middle" dy="12" fontSize="7" fontFamily="sans-serif" fill={faint} letterSpacing="1">{c.label.toUpperCase()}</text>
          </g>
        ))}
        {d && !d.valid && (
